...
/** @var array<string, array{label: string, description: string, logo: string}> $options */
...
foreach ($options as $key => $value) {
    ...
    ?>
<div
	class="<?= $form_check_class ?><?= '' == trim($this->getLabel()) ? $this->getWarningClass() : '' ?>">
	<?php
        $attributes = [
	...
	<label class="d-block"
		for="<?= $this->getFieldId() . '-' . htmlspecialchars($key) ?>">
		<div class="card mb-2 d-block width-100">
			<?php if (!empty($value['logo'])): ?>
			<img src="<?= htmlspecialchars($value['logo']) ?>"
				class="card-img-top" alt="Logo" style="max-height: 80px; object-fit: contain;">
			<?php endif; ?>
