							...
							<table style="position:relative;margin-top:-1cm;border:1px solid #000;">
								<tbody>
									...
									<tr>
										...
										<td><?= $order->getValue('shippment_type') ?>
										</td>
									</tr>
									<tr>
...
?>
			...
			<p><?= sprintf(sprogcard('pdf_order_msg_prepayment'), number_format($total, 2, ',', '.') . ' ' . $currency, $order->getValue('verwendungszweck')); ?>
			</p>
			<?php endif; ?>
		</div>
...
?>
			...
			<h2 class="invoice">
				<?= sprintf(sprogcard('pdf_delivery_note_headline'), $order->getValue('order_number')); ?>
			</h2>
