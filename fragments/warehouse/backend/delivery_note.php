							...
							<table style="position:relative;margin-top:-1cm;border:1px solid #000;">
								<tbody>
									...
									<tr>
										...
										<td><?= $order->getValue(Order::SHIPPMENT_TYPE) ?>
										</td>
									</tr>
									<tr>
...
?>
							...
							<table class="billet" cellspacing="0">
								<tbody>
									<tr>
										...
										<?php if ($with_tax): ?>
										<td><?= number_format($sub_total, 2, ',', '.') . ' ' . $currency ?>
										</td>
										<?php else: ?>
										<td><?= number_format($sub_total_netto, 2, ',', '.') . ' ' . $currency ?>
										</td>
										<?php endif; ?>
									</tr>
									<tr>
			...
			<p><?= sprintf(sprogcard('pdf_order_msg_prepayment'), number_format($total, 2, ',', '.') . ' ' . $currency, $order->getValue(Order::VERWENDUNGSZWECK)); ?>
			</p>
			<?php endif; ?>
		</div>
...
?>
			...
			<h2 class="invoice">
				<?= sprintf(sprogcard('pdf_delivery_note_headline'), $order->getValue(Order::ORDER_NUMBER)); ?>
			</h2>
...
?>
			...
			<p>Ø: Dieses Produkt ist aktuell leider vergriffen und wird Ihnen schnellstmöglich nachgeliefert. Danke für
				Ihre Geduld!</p>
		</div>
	</main>
</body>
