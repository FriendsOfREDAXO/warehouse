...
<table class="uk-table uk-table-striped uk-width-1-1 uk-table-small" id="table_order_summary">
	<thead>
		<tr>
			...
			<th></th>
			<?php if ($with_tax): ?>
			<th class="uk-text-right">{{ tax }}</th><?php endif; ?>
			<th class="uk-text-right">
				<?= Warehouse::getCurrencySign() ?></th>
		</tr>
	</thead>
	...
	<tr>
		<td>
			<div class="uk-text-meta">
				<?= ((isset($item['var_whvarid']) && !empty($item['var_whvarid'])) ? $item['var_whvarid'] : $item['whid'])  ?>
			</div>
			...
			<?= $item['count'] ?> x à
			<?= warehouse::number_format(warehouse::get_item_price($item), 2) ?>
		</td>
		<td><?=((isset($item['var_bezeichnung'])&&!empty($item['var_bezeichnung']))?'<div class="uk-text-meta">&nbsp;</div>'.$item['var_bezeichnung']:'')?>
		</td>
		<?php if ($with_tax): ?>
		...
		<?php endif; ?>
		<td class="uk-text-right">
			<?= warehouse::number_format(warehouse::get_item_price_total($item), 2) ?>
		</td>
	</tr>
	<?php endforeach ?>
	<tr>
		<td colspan="2">{{ Shipping }}
			<?= $this->wh_userdata['country'] ?>
		</td>
		<td colspan="2" class="uk-text-right">
			<?= warehouse::number_format($shipping, 2) ?></td>
	</tr>
	<?php if ($with_tax):
	...
	<tr>
		...
		<td colspan="2" class="uk-text-right">
			<?= warehouse::number_format($taxValue, 2) ?></td>
	</tr>
	<?php endforeach; endif; ?>
	<tr>
		<td colspan="2"><?php if ($with_tax) { ?>{{ Total_Mwst
			}}<?php } else { ?>{{ Total
			}}<?php } ?></td>
		<td colspan="2" class="uk-text-right">
			<?= warehouse::number_format(warehouse::get_cart_total(), 2) ?>
		</td>
	</tr>
</table>
<p>{{ Lieferadresse }}:</p>
...
<p><?= Warehouse::getLabel('payment_type'); ?>:
	{{ payment_<?= $user_data['payment_type'] ?>
	}}</p>
