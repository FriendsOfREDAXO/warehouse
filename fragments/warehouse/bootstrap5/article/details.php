...
<div class="row g-4" data-warehouse-article-detail>
	...
	<div class="col-12 col-md-8">
		...
		<p><?= htmlspecialchars($article->getShortText(true) ?? '') ?>
		</p>
		...
		<div class="mb-3">
			<h4><?= Warehouse::getLabel('bulk_prices'); ?>
			</h4>
			<table class="table table-striped table-bordered">
				<thead>
					<tr>
						<th scope="col">
							<?= Warehouse::getLabel('amount'); ?>
						</th>
						<th scope="col">
		...
		<div class="row g-3">
			...
			<div class="col-12">
				...
				<div data-warehouse-price-display
					...
					<span data-warehouse-price-value
						class="fs-3"><?= $article->getPriceFormatted() ?></span>
					<p class="text-small mb-0">
						<?= Warehouse::getLabel('tax') ?>
						<a href="#shipping_modal"
							data-bs-toggle="modal"><?= Warehouse::getLabel('shipping_costs') ?></a>
					</p>
				</div>
				<!-- / Preis -->
			</div>
			...
			<div class="col-12">
				<form data-warehouse-add-form
					data-warehouse-checkout-url="<?= rex_getUrl(rex_config::get('warehouse', 'address_page')) ?>">
					<input type="hidden" name="article_id"
						value="<?= $article->getId() ?>">
					<div class="input-group mb-3">
						<button class="btn btn-outline-primary" type="button" data-warehouse-quantity-switch="-1"
							data-warehouse-quantity-input="warehouse_count_<?= $article->getId() ?>">[-]</button>
						<input name="order_count" type="number" min="1" step="1" class="form-control"
							id="warehouse_count_<?= $article->getId() ?>"
							data-warehouse-quantity-input value="1">
						<button class="btn btn-outline-primary" type="button" data-warehouse-quantity-switch="+1"
							data-warehouse-quantity-input="warehouse_count_<?= $article->getId() ?>">[+]</button>
					</div>
					<button type="submit" name="submit" value="cart" class="btn btn-secondary"
						data-warehouse-success-text="<?= Warehouse::getLabel('add_to_cart_success') ?>"><?= Warehouse::getLabel('add_to_cart') ?></button>
					<?php if (Warehouse::getConfig('instant_checkout_enabled', 1)) : ?>
					<button type="submit" name="submit" value="checkout"
						class="btn btn-primary"><?= Warehouse::getLabel('checkout_instant') ?></button>
					<?php endif; ?>
				</form>
			</div>
		</div>
